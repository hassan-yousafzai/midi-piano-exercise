...
function playNotes() {
  NOTE_DETAILS.forEach(n => {
    ...
    // we need to disconnect any oscillators we already have on any of the note details and readd them
  ...
  const gain = 1 / activeNotes.length // to determine what percentage volume we need to play at, as otherwise the more keys you press the sound gets louder and louder
...
function startNote(noteDetail, gain) {
  ...
  noteDetail.oscillator = oscillator // save a reference to oscillator on the noteDetail object so we can stop it when they are not making any sound
}
